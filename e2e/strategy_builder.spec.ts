...
test.describe('Strategy Builder E2E Test', () => {
  ...
  test('should render the initial UI correctly', async ({ page }) => {
    ...
    await expect(page.locator('.palette')).toBeVisible(); // Palette 컴포넌트 확인
    await expect(page.locator('button:text("1차 스캔")')).toHaveClass(/active/); // 기본 활성 탭 확인
    await expect(page.locator('h3:text("1차 스캔 캔버스")')).toBeVisible();
  ...
  // TODO: 이 테스트는 현재 UI와 맞지 않아 스킵합니다. 조건 변수 관리 기능이 재설계되면 업데이트해야 합니다.
  test.skip('should allow creating and editing a logic variable', async ({ page }) => {
    // 1. 조건 추가 버튼 클릭
  ...
  test('should build and save a strategy with two scan steps', async ({ page }) => {
    // 1차 스캔 캔버스에 조건 추가
    await page.click('button:text("1차 스캔")');
    await page.click('button:text("close")');
    await page.click('button:text(">")');
    await page.click('button:text("open")');
    await expect(page.locator('.preview-box')).toContainText('1차 스캔: close > open');
    await page.screenshot({ path: 'test-results/04-first-scan-added.png' });

    // 2차 스캔 캔버스로 전환하고 조건 추가
    await page.click('button:text("2차 스캔")');
    await page.click('button:text("volume")');
    await page.click('button:text(">")');
    await page.locator('input[placeholder="숫자 입력"]').fill('10000');
    await page.keyboard.press('Enter');
    await expect(page.locator('.preview-box')).toContainText('2차 스캔: volume > 10000');
    await page.screenshot({ path: 'test-results/05-second-scan-added.png' });

    // 전략 저장
    const strategyName = `E2E_2단계_스캔전략_${Date.now()}`;
    await page.locator('input[placeholder="전략 이름 입력"]').fill(strategyName);

    // API 응답 대신 alert 메시지를 기다립니다.
    page.on('dialog', async dialog => {
      expect(dialog.message()).toContain('전략이 저장되었습니다.');
    ...
    await page.screenshot({ path: 'test-results/06-strategy-saved.png' });
    ...
    // 드롭다운에서 저장된 전략 선택
    await page.locator('select').first().selectOption({ label: strategyName });

    // 상태가 복원되었는지 확인
    await expect(page.locator('.preview-box')).toContainText('1차 스캔: close > open');
    await expect(page.locator('.preview-box')).toContainText('2차 스캔: volume > 10000');
    await page.screenshot({ path: 'test-results/07-strategy-loaded.png' });
  });
